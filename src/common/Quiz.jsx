...
const Quiz = ({ setAnalyticUpdate }) => {
  ...
  const handleClickOption = async (index) => {
    ...
    try {
      const response = await axios.get(`/api/quiz-play`);
    ...
    const sendData = {
      userId: null, // explicitly null since we're not using auth
    ...
    try {
      await axios.post(`/api/quiz-analytic-save`, sendData);
  ...
  const handleNextQuestion = async () => {
    ...
    if (playedQuestion === 5 && !played) {
      try {
        const response = await axios.post(`/api/save-ip-quiz-play`);
  ...
  return (
    <div className='container mx-auto px-4 py-10 text-gray-300'>
      {showPopup && (
        <PopupBox
      ...
      <div className='grid grid-cols-2 gap-10'>
        <div className='col-span-2 flex flex-col items-center gap-7'>
        ...
        <div className='lg:col-span-1 col-span-2 bg-gray3 flex flex-col justify-center items-start px-5 gap-5 border-0 rounded-lg py-10'>
        ...
        <div className='lg:col-span-1 col-span-2 border-2 border-gray2 rounded-lg flex flex-col gap-5 justify-center items-center py-6 px-3'>
          ...
          <button
            className='ml-auto mr-5 py-2 px-4 bg-blue-600 hover:bg-green text-white font-bold rounded-lg transition duration-500 ease-in-out'
...
const QuizOption = ({ label, text, correct, handleClickOption, selected, selectedOption }) => (
  <div
    className={`w-full min-h-20 flex justify-between items-center border-2 border-gray2 rounded-xl py-4 px-6 cursor-pointer transition-colors duration-300 
      ${selected ? (correct ? 'bg-light-green border-green' : 'bg-red1 border-red') : ''} 
      ${selectedOption !== null && correct ? 'bg-light-green border-green' : ''}`}
