...
// Define types for the admin data
interface Admin {
  id: number;
  name: string;
  email: string;
...
export default function AdminTable() {
  const [admins, setAdmins] = useState<Admin[]>([]); // State to hold admin data
  const [loading, setLoading] = useState<boolean>(true); // Loading state
  const [currentPage, setCurrentPage] = useState<number>(0); // Current page
  const adminsPerPage = 15; // Number of admins per page
  ...
  const fetchAdmins = async () => {
    try {
      ...
      if (!res.ok) {
        throw new Error(`HTTP error! status: ${res.status}`);  // Throw error if status is not ok (non-2xx)
      }
  ...
  // Handle admin deletion (Optional)
  const deleteAdmin = (adminId: number) => {
    const confirmDelete = confirm('Are you sure you want to delete this admin?');
    if (!confirmDelete) return;

    setAdmins((prev) => prev.filter((admin) => admin.id !== adminId));
    // Optionally, you can add an API call for deleting the admin
  };

  // Paginate admins for current page
