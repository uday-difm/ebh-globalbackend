...
export const CategorySlider = ({ categories }) => {
  ...
  const ArrowButton = ({ onClick, children, isDisabled }) => (
    <div
      ...
      className={`w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-md transition-opacity ${
        isDisabled
        ...
        if (isAnimationEnabled) {
            return (
                <Link href={`/blogs/${blog.blog_slug}`} className="block max-w-[1350] group">
                    <div className={`relative h-[24rem] transform transition-transform duration-1000 ${isHovered ? "-translate-y-4" : ""}`} onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={() => setHoveredIndex(null)}>
